...
from pymacro.db.database import db
from pymacro.db.documents import IngredientDocument
from pymacro.db.error_handler import BaseErrorHandler
...
class BaseCollection:
    ...
    def _execute_with_error_handling(
        func: Callable, error_handler: BaseErrorHandler, *args, **kwargs
    ) -> Any:
        try:
    ...
    def _validate_docs(self, docs: list[dict]) -> list[dict]:
        ...
        return self._execute_with_error_handling(
            func=aux_func, error_handler=self.error_handler, docs=docs
        )

    def _execute_find(self, query: dict, pipeline_steps: list = []) -> list[dict]:
    ...
    def _execute_insertion(
        self, func: Callable, docs: Union[dict, list[dict]], find_query: dict
        ...
        return self._execute_with_error_handling(
            func=insert_documents, error_handler=self.error_handler, docs=docs
        )
    ...
    def _execute_update(self, func, query: dict, update_data: dict):
        ...
        if result.matched_count > 0:
            return (
                self.find_one(query)
                if func == self.collection.update_one
                else self.find(query)
            )
    ...
    def _execute_delete(self, func, query: dict):
        result = self._execute_with_error_handling(
            func=func, error_handler=self.error_handler, filter=query
        )
        return result

    def _handle_result(
        self, result: list[dict], func: Callable | None = None, validate: bool = True
    ...
    def find_one(self, query: dict, pipeline_steps: list = []) -> Optional[dict]:
        result = self._execute_find(query=query, pipeline_steps=pipeline_steps)
    ...
    def insert_by_id(
        self, document_id: ObjectId, document_data: dict
    ) -> Optional[dict]:
    ...
    def update_one(self, query: dict, update_data: dict) -> Optional[dict]:
        return self._execute_update(
            func=self.collection.update_one, query=query, update_data=update_data
        )

    def update_many(self, query: dict, update_data: dict) -> list[dict]:
        return self._execute_update(
            func=self.collection.update_many, query=query, update_data=update_data
        )
    ...
    def delete_one(self, query: dict) -> int:
        return self._execute_delete(func=self.collection.delete_one, query=query)

    def delete_many(self, query: dict) -> int:
        return self._execute_delete(func=self.collection.delete_many, query=query)

    def delete_by_id(self, document_id: ObjectId) -> int:
        return self.delete_one({"_id": document_id})

class IngredientsCollection(BaseCollection):
    def __init__(self):
        super().__init__(db["ingredients"], IngredientDocument)

    def create_indexes(self):
        self.collection.create_index([("name", 1)])
        self.collection.create_index([("category", 1)])
        self.collection.create_index([("carbs", 1)])
        self.collection.create_index([("protein", 1)])
        self.collection.create_index([("fat", 1)])
        self.collection.create_index([("calories", 1)])
